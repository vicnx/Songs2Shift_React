...
const SSSidebarMenu = () => {
	...
	return (
		<div>
			<Button icon="pi pi-bars" className="p-button-rounded p-button-secondary" onClick={toggleMenu} style={{ position: 'absolute', right: 0 }} />
			<Sidebar visible={visible} showCloseIcon={false} onHide={() => setVisible(false)} className="p-sidebar-menu">
				<div className="p-d-flex p-flex-column p-ai-center">
					...
					<div className="p-mt-6">
						{items.map((item, index) => (
							<Button key={index} label={item.label} icon={item.icon} className={item.className} />
						))}
					</div>
				</div>
			</Sidebar>
